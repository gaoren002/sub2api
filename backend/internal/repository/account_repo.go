...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, imageQuota string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if imageQuota == service.AccountImageQuotaExhaustedFilter {
		q = q.Where(dbpredicate.Account(func(s *entsql.Selector) {
			exhaustedPath := sqljson.Path("openai_images_quota_exhausted")
			resetPath := sqljson.Path("openai_images_quota_reset_at")
			s.Where(entsql.And(
				sqljson.ValueEQ(dbaccount.FieldExtra, true, exhaustedPath),
				sqljson.ValueGT(dbaccount.FieldExtra, time.Now().UTC().Format(time.RFC3339), resetPath),
			))
		}))
	}
