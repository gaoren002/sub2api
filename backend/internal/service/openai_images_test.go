...
func TestOpenAIGatewayServiceForwardImages_OAuthStreamingPreservesStructuredImageError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","stream":true}`)
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(
				`data: {"type":"error","error":{"code":"moderation_blocked","message":"Your request was rejected by the safety system.","param":null,"type":"image_generation_user_error"}}` + "\n\n",
			)),
		},
	}
	svc.httpUpstream = upstream
	...
	event, ok := findOpenAIImageTestSSEEvent(parseOpenAIImageTestSSEEvents(rec.Body.String()), "error")
	require.True(t, ok)
	...
	require.True(t, gjson.Get(event.Data, "error.param").Exists())
	require.Equal(t, gjson.Null, gjson.Get(event.Data, "error.param").Type)
	require.Equal(t, "Your request was rejected by the safety system.", gjson.Get(event.Data, "error.message").String())
}
